...
const HomePage = () => {
  ...
  const fetchServices = async (city) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/services/get?city=${city}`);
        ...
        <Link to={`/service/${service._id}`} className="btn btn-warning">
          View Details
                        ...
                        <div className="col-md-6 d-flex align-items-center justify-content-center">
                          <Card.Img variant="top" src={`http://localhost:5000/${imageName}`} className="card-img-top img-fluid" />
                        </div>
                      </div>
                    </Card>
                  </Carousel.Item>
