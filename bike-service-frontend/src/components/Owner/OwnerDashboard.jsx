...
const OwnerDashboard = () => {
  ...
  const fetchServices = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/services?ownerEmail=${ownerEmail}&city=${city}`);
  ...
  const fetchBookings = async (city) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/bookings?city=${city}`);
  ...
  const handleViewMessages = async () => {
    try {
      const ownerEmail = localStorage.getItem('ownerEmail');  
       const city = localStorage.getItem('ownerCity'); 
      const response = await axios.get(`http://localhost:5000/api/contact?owneremail=${ownerEmail}&city=${city}`);
